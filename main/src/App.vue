<template>
  <div class="layout-wrapper">
    ...
    <div class="layout-left">
      <ul class="newsList">
        <li v-for="item in news" :class="{active: item.activeRule === current}" :key="item.name" @click="goto(item)">{{ item.name }}</li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    bindCurrent () {
      ...
      if (this.news.findIndex(item => item.activeRule === path) >= 0) {
        this.current = path
      }
    },
    listenRouterChange () {
